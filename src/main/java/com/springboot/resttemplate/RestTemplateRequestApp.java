...
public class RestTemplateRequestApp 
{
	...
	{
	    final String uri = "http://localhost:8080/api/employees";
	...
	public String deleteRecord(@PathVariable(value="id") int id) 
	{
		//ModelAndView model = new ModelAndView();
		 final String uri = "http://localhost:8080/api/deletemployee/"+id;
	...
	public String insertRecord(@PathVariable("id") int id, @PathVariable("name") String name,
			@PathVariable("addrs") String addrs,
			@PathVariable("finished") String finished/* ,@PathVariable("date") String date */) throws Exception
	{
		
		RestTemplate restTemplate = new RestTemplate();
		final String url = "http://localhost:8080/api/createmployee";
		Task user = new Task();
		user.setId(id);
		user.setName(name);
		user.setAddrs(addrs);
		...
		Task addedUser = restTemplate.postForObject(url, user, Task.class);
		 ...
		 final String url1 = "http://localhost:8080/api/employees";
	...
	{
		//ModelAndView model = new ModelAndView();
		final String uri = "http://localhost:8080/api/updatemployee/"+id;
	
		RestTemplate restTemplate = new RestTemplate();
		final String url = "http://localhost:8080/api/updatemployee/"+id;
		Task user = new Task();
		user.setId(id);
		user.setName(name);
		user.setAddrs(addrs);
