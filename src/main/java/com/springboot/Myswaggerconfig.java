...
public class Myswaggerconfig {
	...
	public Docket postsApi() {
		return new Docket(DocumentationType.SWAGGER_2).apiInfo(metadata()).select().paths(regex("/rest.*")).build();
	}

	@SuppressWarnings("deprecation")
	private ApiInfo metadata() {
		return new ApiInfoBuilder().title("Java Code Geeks").description("API reference guide for developers").termsOfServiceUrl("https://www.balaji.com/").contact("Batra, Yatin").version("1.0").build();	
	}
}